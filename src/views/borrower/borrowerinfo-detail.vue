...
export default {
  ...
  created() {
    this.getBorrowerInfoDetail();
  },
  methods: {
    back() {
      this.$router.push({ path: "/borrower/infoList" });
    },
    getBorrowerInfoDetail() {
      borrowerInfoApi.show(this.$route.params.id).then((r) => {
        this.borrowInfoDetail = r.data.result;
      });
    },
  },
};
